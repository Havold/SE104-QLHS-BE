...
export const getAllSemesters = async (req, res) => {
  ...
  try {
    const count = await prisma.semester.count({ where: query });
    if (p * pItems > count) {
    ...
    if (type === "all") {
      ...
      res.status(200).json({ semesters, totalCount: count, currentPage: 1 });
      return;
    }
    ...
    res.status(200).json({ semesters, totalCount: count, currentPage: p });
  } catch (error) {
    console.log(error);
